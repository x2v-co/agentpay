...
function render() {
  ...
  if(form) {
    ...
    slider.addEventListener('input',()=>{form.querySelector('#budget-label').textContent=`${money([0,10000,100000][slider.value])} USDC`;});
    ...
    form.addEventListener('submit',e=>{e.preventDefault(); const budget=[0,10000,100000][slider.value];const mode=new FormData(form).get('mode');const checkpoint=state.checkpoint;const runId=state.runId||crypto.randomUUID();state={...freshState(),hired:true,budget,mode,runId,checkpoint};go(checkpoint?'market':'work');});
  }
  if (current === 'connect') {
...
app.addEventListener('click',e=>{
  ...
  if(action==='resume'){go(state.paymentDone?'delivery':state.checkpoint?'market':'work');return;}
  ...
  if(action==='accept'){state.accepted=true;save();render();return;}
  ...
  if(action==='try-ask'||action==='try-budget'){state={...state,mode:action==='try-ask'?'ask':'auto',budget:action==='try-budget'?10000:100000,approved:false,denied:false,paymentDone:false,tested:false,accepted:false};go('market');return;}
